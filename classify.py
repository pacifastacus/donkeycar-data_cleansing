...
class App(tk.Frame):
    ...
    def open_dir(self):
        self.saveCallback()
        foo = filedialog.askdirectory(mustexist=True, title="Select Directory")
        app.datadir = foo
...
if __name__ == '__main__':
    ...
    root.resizable(0,0)
    app = App(data=(img_dir,data), master=root,saveCallback=save)
    app.master.title(__file__+" : "+img_dir)

    mainmenu = tk.Menu(root)
    #mainmenu.add_command(label="Save (Ctrl+S)",command=save)
    mainmenu.add_command(label="Open Directory (Ctrl+O)", command=app.open_dir)
